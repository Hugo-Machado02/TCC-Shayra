...
$validar = mysqli_query($conexao, "SELECT log.usuario_id, fun.cpf, fun.nome_funcao FROM login as log INNER JOIN funcionario as fun ON (log.usuario_id = fun.usuario_id)
WHERE log.usuario = '{$usuario}' AND log.senha = '{$senha}'");

$funcao = mysqli_fetch_assoc($validar);
$_SESSION['usuario_id'] = $funcao['usuario_id'];
...
if($row == 1) {
	...
	$_SESSION['Nascimento_Med'] = $usuario_bd['dt_nasc'];
